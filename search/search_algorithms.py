...
import os, csv
import csv, search.DFS as DFS, search.BFS as BFS, search.ITERATIVE as ITERATIVE, search.UCS as UCS, search.GREEDY as GREEDY, search.ASTAR as ASTAR
...
def a_star_search(map_data, start, end):
    ...
    ASTAR.instructions = []
    ASTAR.coor_list = []
    ASTAR.open = PriorityQueue()
    ASTAR.close = []
    ASTAR.ASTAR(map_data, start, end)


# Funcion para leer el archivo csv
def read_csv(file_name):
    map_data = []
    with open(os.path.join(os.path.dirname(__file__)[:-6] + '/mazes', file_name), 'rt') as f:
            reader = csv.reader(f)
            for row in reader:
                if row:
                    map_data.append(row)
...
def solve(file_name, mode, depth=0):
    ...
    if mode == 'iter':
        ...
        a_star_search(map_data[:], start, end)
        coor_list = ASTAR.coor_list

        ASTAR.update_instructions()

        return [t[::-1] for t in coor_list[1:]], ASTAR.instructions
